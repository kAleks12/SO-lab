...
while getopts ":hd:t:af:x" OPT; do
    case ${OPT} in
        h)
            print_help
            exit 0
        ...
        d)
            ...
            if ! [[ -d ${MOVIES_DIR} ]]; then
                print_error "ERROR: Provided movie directory doesn't exist!"
                exit 1
            fi
            ;;
        t)
...
if ! [ -v MOVIES_DIR ]; then
    print_error "ERROR: required flag -d not provided!"
    exit 1
fi
